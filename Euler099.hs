...
computeLog :: String -> Double
computeLog s = b * log a
  where (sa,sb) = break (==',') s
        a = read sa :: Double
        b = read $ tail sb :: Double

result099 = fmap (fst . maximumBy (comparing snd) . zip [1..] . map computeLog . lines) $ readFile "data/base_exp.txt"
