...
decrypt message key = map chr $ zipWith xor message $ cycle key

getPosiblePlains :: [Int] -> [String]
getPosiblePlains message = filter (all isPrint) [decrypt message [a,b,c] | a <- lowChrs, b <- lowChrs, c <- lowChrs]

getPlain :: [String] -> String
getPlain = maximumBy (comparing (length . filter isLetter)) 
...
result059 = do
  ...
  return . sum . map ord . getPlain . getPosiblePlains . map read $ splitStr content
