...
leftNearest :: (Int, Int) -> Int -> Int
leftNearest (hiN,hiD) d
  | d == hiD = helper (hiN - 1)
  | otherwise = helper (d * hiN `div` hiD)
  where helper 0 = 0
        helper x
          | gcd x d == 1 = x
          | otherwise = helper (x-1)

result071 = snd . maximumBy (\(a,b) (c,d) -> compare (b * c) (a * d)) . zip [3..] $ map (leftNearest (3,7)) [3..1000000]
