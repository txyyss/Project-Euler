...
regions = [(c, i) | c <- [1..3], i <- [1..20]] ++ [(1, 25), (2, 25)]

score (c,a) = c * a

result109 = length checkouts
  where checkouts = [lst | lst <- checkout1 ++ checkout2 ++ checkout3, let s = sum $ map score lst, s < 100]
        checkout1 = [[x] | x <- regions, fst x == 2]
        checkout2 = [[x, y] | x <- regions, y <- regions, fst y == 2]
        checkout3 = [[x, y, z] | x <- regions, y <- regions, x <= y, z <- regions, fst z == 2]
